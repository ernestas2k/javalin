...
class WsPathMatcher {
    ...
    fun webSocketConnect(session: Session) {
        findEntry(session)?.let {
            val ctx = WsContextOnConnect(sessionIds[session]!!, session, sessionPathParams[session]!!, it.path)
    ...
    fun webSocketMessage(session: Session, message: String) {
        findEntry(session)?.let {
            val ctx = WsContextOnMessage(sessionIds[session]!!, session, sessionPathParams[session]!!, it.path, message)
    ...
    fun webSocketBinaryMessage(session: Session, buffer: ByteArray, offset: Int, length: Int) {
        findEntry(session)?.let {
            val ctx = WsContextOnBinaryMessage(sessionIds[session]!!, session, sessionPathParams[session]!!, it.path, buffer.toTypedArray(), offset, length)
    ...
    fun webSocketError(session: Session, throwable: Throwable?) {
        findEntry(session)?.let {
            val ctx = WsContextOnException(sessionIds[session]!!, session, sessionPathParams[session]!!, it.path, throwable)
    ...
    fun webSocketClose(session: Session, statusCode: Int, reason: String?) {
        findEntry(session)?.let {
            val ctx = WsContextOnClose(sessionIds[session]!!, session, sessionPathParams[session]!!, it.path, statusCode, reason)
